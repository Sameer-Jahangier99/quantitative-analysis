...
class HybridRetriever:
    ...
    def query(self, query: str) -> Dict:
        ...
        Returns dict with retrieved contexts and metadata
        """
        ...
        retrieved_contexts = [text for text, score in sorted_results[:top_k]]
        ...
        return {
            ...
            "total_tokens": sum(len(ctx.split()) for ctx in retrieved_contexts)  # Rough estimate
        }
...
class CAGEnhancedRAG:
    ...
    def add_documents(self, documents: List[str]):
        ...
        # Update caches (prompt and semantic)
        for doc in documents:
            self.cag_layer.prompt_cache.set(doc, doc)
            embedding = self.retriever.embedding_model.encode([doc]).tolist()
            self.cag_layer.semantic_cache.set(embedding[0], doc)
        
        print(f"✓ Added {len(documents)} documents to system caches")
    
    def query(self, query: str) -> Dict:
        """Perform query with retrieval and generation"""
        start_time = time.perf_counter()
        ...
        # Step 1: Check Prompt Cache (exact match)
        cached_result = self.cag_layer.prompt_cache.get(query)
        
        if cached_result and isinstance(cached_result, str):
            # Cache hit - result is the answer string
            cache_hit = True
            cache_type = "prompt"
            answer = cached_result
            print(f"  ✓ Prompt cache hit!")
        
        # Step 2: Check Semantic Cache (fuzzy/similarity match)
        if not cache_hit and self.retriever.embedding_model:
            try:
                ...
                if semantic_result and isinstance(semantic_result, str):
                    ...
                    print(f"  ✓ Semantic cache hit (similarity: {self.cag_layer.semantic_cache.similarity_threshold})")
            except Exception as e:
        ...
        if not cache_hit:
            ...
            top_contexts = retrieval_results["retrieved_contexts"]
            
            # Generate answer from LLM
            prompt = self._build_prompt(query, top_contexts)
            answer, generation_time_ms, prompt_tokens, generation_tokens = self.llm_engine.generate(prompt)
            
            # Cache the answer in both caches
            self.cag_layer.prompt_cache.set(query, answer)
            
            # Add to semantic cache for fuzzy matching
            if self.retriever.embedding_model:
                try:
                    query_embedding = self.retriever.embedding_model.encode([query])[0]
                    self.cag_layer.semantic_cache.set(query_embedding, answer)
                except Exception as e:
                    print(f"  ⚠ Failed to add to semantic cache: {e}")
        
        end_time = time.perf_counter()
        total_latency_ms = (end_time - start_time) * 1000
        
        # Save metrics
        self._save_query_metrics(query, answer, total_latency_ms, cache_hit, cache_type)
        
        # Use retrieval results if available, otherwise use defaults
        ...
        return {
            ...
            "retrieval_latency_ms": retrieval_results["retrieval_latency_ms"] if not cache_hit else 0,
            "generation_latency_ms": generation_time_ms if not cache_hit else 0,
    ...
    def _build_prompt(self, query: str, contexts: List[str]) -> str:
        ...
        prompt += "Relevant Contexts:\n"
        
        for i, context in enumerate(contexts):
            prompt += f"  - Context {i+1}: {context}\n"
        
        prompt += "\nAnswer the query based on the above contexts."
        
        return prompt
    
    def _save_query_metrics(self, query: str, answer: str, total_latency_ms: float, cache_hit: bool, cache_type: Optional[str] = None):
        ...
        metrics = CAGQueryMetrics(
            ...
            semantic_cache_checked=cache_type == "semantic",
            prompt_tokens=0,
            context_tokens=0,
            generation_tokens=0,
            total_tokens=0,
        ...
        print(f"✓ Metrics saved for query {query_id}: {query[:50]}...")
    
    def save_metrics(self, filepath: str):
        """Save all metrics to file"""
        self.metrics_monitor.save_metrics(filepath)
    def get_statistics(self) -> Dict:
        """Return execution statistics in format compatible with comparison script"""
        return self.metrics_monitor.get_summary_statistics()
    def print_statistics(self):
        """Print combined statistics from all layers"""
        stats = self.cag_layer.get_all_stats()
        ...
        for layer, stat in stats.items():
            ...
            for key, value in stat.items():
                if isinstance(value, float):
                    print(f"  {key}: {value:.4f}" if value < 1 else f"  {key}: {value}")
                else:
                    print(f"  {key}: {value}")
