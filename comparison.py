...
class SystemComparison:
    ...
    def run_comparison(self, documents: List[str], questions: List[str],
        ...
        if include_duplicates:
            ...
            # Add 30% duplicates and 20% semantic variations
            num_duplicates = int(len(questions) * 0.3)
            num_variations = int(len(questions) * 0.2)
            
            duplicates = np.random.choice(questions, num_duplicates, replace=False).tolist()
            ...
            for q in np.random.choice(questions, num_variations, replace=False):
                if q.startswith("What"):
                    ...
                    variations.append(q.replace("How", "Describe how"))
                else:
    ...
    def _compute_comparison(self):
        ...
        latency_improvement_p50 = ((vanilla_p50 - cag_p50) / vanilla_p50) * 100
        
        vanilla_p95 = vanilla['latency']['total']['p95']
        cag_p95 = cag['latency']['all_queries']['p95']
        latency_improvement_p95 = ((vanilla_p95 - cag_p95) / vanilla_p95) * 100
        
        vanilla_mean = vanilla['latency']['total']['mean']
        cag_mean = cag['latency']['all_queries']['mean']
        latency_improvement_mean = ((vanilla_mean - cag_mean) / vanilla_mean) * 100
        
        # Token savings
        vanilla_tokens = vanilla['tokens']['total']['sum']
        cag_tokens = cag['tokens']['total']['sum']
        token_reduction = ((vanilla_tokens - cag_tokens) / vanilla_tokens) * 100
        
        # Memory comparison
        vanilla_cpu_peak = vanilla['memory']['cpu']['peak_mb']
        cag_cpu_peak = cag['memory']['cpu']['peak_mb']
        cpu_memory_overhead = ((cag_cpu_peak - vanilla_cpu_peak) / vanilla_cpu_peak) * 100
        
        vanilla_gpu_peak = vanilla['memory']['gpu']['peak_mb']
        cag_gpu_peak = cag['memory']['gpu']['peak_mb']
        gpu_memory_overhead = ((cag_gpu_peak - vanilla_gpu_peak) / vanilla_gpu_peak) * 100
        
        # Throughput comparison
        vanilla_qps = vanilla['throughput']['queries_per_second']
        cag_qps = cag['throughput']['queries_per_second']
        throughput_improvement = ((cag_qps - vanilla_qps) / vanilla_qps) * 100
        ...
        self.comparison_data = {
            ...
            "memory": {
                "cpu": {
                    "vanilla_peak_mb": vanilla_cpu_peak,
                    "cag_peak_mb": cag_cpu_peak,
                    "overhead_percent": cpu_memory_overhead
                },
                "gpu": {
                    "vanilla_peak_mb": vanilla_gpu_peak,
                    "cag_peak_mb": cag_gpu_peak,
                    "overhead_percent": gpu_memory_overhead
                }
            },
            "throughput": {
    ...
    def _print_comparison(self):
        ...
        print("\n--- MEMORY COMPARISON ---")
        print(f"CPU RAM (Peak):")
        print(f"  Vanilla RAG: {comp['memory']['cpu']['vanilla_peak_mb']:.2f}MB")
        print(f"  CAG RAG:     {comp['memory']['cpu']['cag_peak_mb']:.2f}MB")
        print(f"  Overhead:    {comp['memory']['cpu']['overhead_percent']:.2f}%")
        
        print(f"\nGPU VRAM (Peak):")
        print(f"  Vanilla RAG: {comp['memory']['gpu']['vanilla_peak_mb']:.2f}MB")
        print(f"  CAG RAG:     {comp['memory']['gpu']['cag_peak_mb']:.2f}MB")
        print(f"  Overhead:    {comp['memory']['gpu']['overhead_percent']:.2f}%")
    ...
    def _generate_visualizations(self):
        ...
        fig.add_trace(
            go.Bar(
                name='CPU',
                x=['Vanilla RAG', 'CAG RAG'],
                y=[comp['memory']['cpu']['vanilla_peak_mb'],
                   comp['memory']['cpu']['cag_peak_mb']],
        ...
        fig.add_trace(
            go.Bar(
                name='GPU',
                x=['Vanilla RAG', 'CAG RAG'],
                y=[comp['memory']['gpu']['vanilla_peak_mb'],
                   comp['memory']['gpu']['cag_peak_mb']],
